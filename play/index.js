...
play('Carousel', module)
	.add('Default', h =>
		createContainer(h, containerWidth, [h(Carousel, {}, generateSlideImages(h))])
	)
	.add('3 per page', h =>
		createContainer(h, containerWidth, [
			h(Carousel, { props: { perPage: 3 } }, generateSlideImages(h))
		])
	)
	.add('Scroll per page false', h =>
		createContainer(h, containerWidth, [
			h(Carousel, { props: { scrollPerPage: false } }, generateSlideImages(h))
		])
	)
	.add('Responsive', h =>
		createContainer(h, containerWidth, [
			h(
				Carousel,
				{ props: { perPageCustom: [[480, 3], [768, 4]] } },
	...
	.add('Autoplay, pause on hover', h =>
		createContainer(h, containerWidth, [
			h(
				Carousel,
				{ props: { autoplay: true, autoplayHoverPause: true } },
				generateSlideImages(h)
			)
		])
	)
	...
	.add('With navigation buttons', h =>
		createContainer(h, containerWidth, [
			h(Carousel, { props: { navigationEnabled: true } }, generateSlideImages(h))
		])
	)
	.add('Navigation buttons and scrollPerPage false', h =>
		createContainer(h, containerWidth, [
			h(
				Carousel,
				{ props: { navigationEnabled: true, scrollPerPage: false } },
				generateSlideImages(h)
			)
		])
	)
	.add('Navigation buttons and scrollPerPage false and loop', h =>
		createContainer(h, containerWidth, [
			h(
				Carousel,
				{
					props: { navigationEnabled: true, scrollPerPage: false, loop: true }
				},
				generateSlideImages(h)
			)
		])
	)
	.add('With customized navigation buttons', h =>
		createContainer(h, containerWidth, [
			...
			h(
				...
				{
					props: {
						paginationColor: '#fac232',
						paginationActiveColor: '#c9750c',
	...
	.add('With spacePadding 100px', h =>
		createContainer(h, containerWidth, [
			h(
				Carousel,
				{ props: { spacePadding: 100, perPage: 1 } },
				generateSlideImages(h)
			)
		])
	)
	...
	.add('Custom width', {
		...
		render: h => {
			return createContainer(h, containerWidth, [
				h(
					Carousel,
					{
						props: {
